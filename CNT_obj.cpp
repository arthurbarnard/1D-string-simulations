...
#include <cstdlib>
	...
	void CNT_obj::update_sig() 
	{
		for(i=0;i<N_cnt/6;i++)
		{
			sig_cnt[i]=sqrt((6*gam_cnt[i]*kb*temp_cnt*h_cnt)/(massarray[i]));
		}
	}
...
{
	ifstream massfile(filename, ios::in);
...
{
	ifstream K_cnt_file(filename,ios::in);
...
{
	ifstream kap_cnt_file(filename,ios::in);
